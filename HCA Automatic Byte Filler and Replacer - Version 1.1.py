...
def ComparingFiles():
    ...
    if len(displayed_source_files) != len(displayed_replacement_files):
        ...
        while file_index < len(displayed_source_files):
            s_name = os.path.basename(displayed_source_files[file_index])
            r_name = os.path.basename(displayed_replacement_files[file_index])
            s_file = os.path.getsize(displayed_source_files[file_index])
            r_file = os.path.getsize(displayed_replacement_files[file_index])
            ...
            if r_file <= s_file:
                source_element.config(bg='green', text='OK | {2} | {0} | {1}'.format(s_name, ByteOperation(s_file), (file_index + 1)) )
                replace_element.config(bg='green', text='OK | {2} | {0} | {1}'.format(r_name, ByteOperation(r_file), (file_index + 1)) )
                fillable_files += 1
            elif r_file > s_file:
                source_element.config(bg='red', text='NOPE | {2} | {0} | {1}'.format(s_name, ByteOperation(s_file), (file_index + 1)) )
                replace_element.config(bg='red', text='NOPE | {2} | {0} | {1}'.format(r_name, ByteOperation(r_file), (file_index + 1)) )
            file_index+=1
...
def DeleteAllLabels(id):
    ...
    up_arrow.config(state = DISABLED, bg = 'gray')
...
def MoveFilesInSection(increment, id):
    ...
    if down_arrow.cget('state') == 'normal':
        if id == 0:
...
def OptionsWindow():
    ...
    decimal_position_button = Button(options_frame, font='Helvetica 9', bg='#FFFFFF', relief=RIDGE, width=35, bd=5, pady = 0, command=lambda : SetDecimalPosition(False))
    if round_to_decimal_position < 0:
    ...
    automatically_sort_files_checkbox = Checkbutton(options_frame, variable=automatically_sort_files_check_bool, text='Automatically sort added files?', font='Helvetica 9', bg='#CCCCCC', relief=RIDGE, width=33, bd=5, padx=0, pady = 0, command=SetAutomaticFileSort)
    automatically_sort_files_checkbox.grid(row=7, column=0)


    global search_string_button
    search_string_button = Button(options_frame, text='Change Search String:\n{0}'.format(replacing_search_string), font='Helvetica 9', bg='#FFFFFF', relief=RIDGE, width=35, bd=5, pady = 0, command=lambda : SetSearchString(False))
    ...
    debug_mode_checkbox = Checkbutton(options_frame, variable=debug_mode_check_bool, text='Use Debug Mode?', font='Helvetica 9', bg='#CCCCCC', relief=RIDGE, width=33, bd=5, padx=0, pady = 1, command=SetDebugMode)
...
def UseNumberInFileName(): #5
    ...
    OverwriteOptionsData('number_in_filename_used', str(number_in_filename_check_bool.get()) )
...
def SetDebugMode(): #9
    ...
    OverwriteOptionsData('debug_mode', str(debug_mode_check_bool.get()) )

...
def PickedFiles(chosen_directory, special_condition):
    if special_condition == True:
        ...
        chosen_files = tkFileDialog.askopenfilenames(initialdir=chosen_directory, title='Select the .hca file(s)', filetypes=(('HCA File', '*.hca'), ('All Files', '*.*')))#sorted(  )	
    return chosen_files
...
def CreateNewLabel(list_to_check, id_number):
    ...
    for element in list_to_check:
        if element in temp_display: #This updates the file sizes and makes sure duplicate labels aren't created (and at the same time deleted immediately within the UpdateLabels() function)
            ...
            if item_in_label_list >= 32: #This makes sure that any labels that can't be seen (at full screen: 1920x1080) won't be created.
                continue
            ...
            temp_label_list[item_in_label_list].pack()
...
def BaseNumberSort(list_element): #https://www.programiz.com/python-programming/methods/built-in/sorted
    ...
    def FileNameNumberFound():
        ...
        try:
            ...
            return element_name #return the files how they are presented on the windows explorer (by defualt)
...
def RemoveDuplicates(checked_list, id_number):
    temp_list = list(OrderedDict.fromkeys(checked_list)) #list(set(checked_list)) # #https://stackoverflow.com/questions/7961363/removing-duplicates-in-lists
    #print('----------\ntemp')
    #print(temp_list)
    #print('\n----------')
...
def UpdateLabels(offset, id_number):
    ...
    while label_index < len(section_label_list):
        ...
        #Moving Up From End of List
            ...
            section_label_list.append(Label(container, bg=color, anchor='w', wraplength=322, justify="right", padx=4, pady = 2, relief=RAISED, width=40, text='{2} | {0} | {1}'.format(os.path.basename(section_displayed_files[i]), ByteOperation(os.path.getsize(section_displayed_files[i])), (i + 1) ), font='Helvetica 10' ))
            section_label_list[-1].pack()
            #print('press {} {} {}'.format(i, len(section_displayed_files), len(section_label_list)))
            continue
        #Moving Up or Down
        index_size = os.path.getsize(section_displayed_files[i])
        section_label_list[label_index].config(bg=color, text='{2} | {0} | {1}'.format(os.path.basename(section_displayed_files[i]), ByteOperation(index_size), (i+1) ))
        i += 1
        label_index += 1
...
def MoveArrowsActivation(arrow_direction, set_active_state, arrow_section, arrow_section_color='black'): #https://www.geeksforgeeks.org/default-arguments-in-python/
    ...
    if set_active_state == True:
        ...
        arrow2.config(bg='#DE01FE', activebackground='purple', state=NORMAL)
...
#----- .place() calculations -----#
f1_div_3 = (Decimal(1.0/3)/Decimal(10))
f2_div_3 = (Decimal(2.0/3)/Decimal(10))

s_n_r_relwidth = Decimal(0.44) + (f2_div_3/Decimal(10)) #https://stackoverflow.com/questions/10768724/why-does-python-return-0-for-simple-division-calculation
replace_relx = Decimal(0.50) + f1_div_3
mid_relx = Decimal(0.40) + f2_div_3
top_section_relwidth = Decimal(0.8) + f1_div_3 #0.8125
top_section_rely = 0.03125 #Decimal(0.041) + (f2_div_3/Decimal(100))
text_relheight = Decimal(0.1041) + (f2_div_3/Decimal(1000))
...
source_frame = Frame(root_window) #source_frame.place(relwidth=0.45, relheight=0.89, relx=0.015, rely=0.01, anchor='nw' )
source_frame.place(relwidth=s_n_r_relwidth, relheight=top_section_relwidth, relx=0.02, rely=top_section_rely) #relwidth=0.45, relheight=0.8, relx=0.245, rely=0.05, anchor='n'
...
replace_frame.place(relwidth=s_n_r_relwidth, relheight=top_section_relwidth, relx=replace_relx, rely=top_section_rely) #relwidth=0.449, relheight=0.8, relx=0.755, rely=0.05, anchor='n'
...
middle_frame.place(relwidth=f2_div_3, relheight=top_section_relwidth, relx=mid_relx, rely=top_section_rely) #relwidth=0.055, relheight=0.8, relx=0.5, rely=0.05, anchor= 'n'

add_button = Button(middle_frame, bg='orange', text='ADD', font='Helvetica 10', activebackground='purple', command=ConfirmFileSection)
add_button.place(relwidth=1, relheight=0.1, rely=0.40) #rely=0.35

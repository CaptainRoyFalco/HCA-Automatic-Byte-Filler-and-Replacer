...
def WindowCenterPositioner(window, window_width, window_height):
    ...
    if window == root_window:
        ...
        root_window.attributes('-disabled', True)   #Makes it so the user cannot use the root window unless they close the toplevel window  #2nd Answer (By Ronald) https://stackoverflow.com/questions/62395920/how-to-sound-a-bell-when-the-user-clicks-outside-a-modal-window
        window.bind('<Destroy>', lambda e: OnWindowClose(e, window))
...
#When the TopLevel window is closed, this re-enables the root window.
def OnWindowClose(e, window_parameter):
    if e.widget == window_parameter: #https://stackoverflow.com/questions/58328686/destroy-events-executes-the-binded-function-5-times
        root_window.attributes('-disabled', False)
    pass
...
def CreateNewLabel(list_to_check, id_number):
    ...
    item_in_label_list = len(temp_label_list) - 1
    ...
    compare_button.config(state=NORMAL, bg='yellow')
...
def MoveArrowsActivation(arrow_direction, set_active_state, arrow_section, arrow_section_color='black'): #https://www.geeksforgeeks.org/default-arguments-in-python/
    ...
    if set_active_state == True:
        ...
        if (up_condition == True and source_up_arrow_button.cget('state') == 'disabled' and replace_up_arrow_button.cget('state') == 'disabled') or (up_condition == False and source_down_arrow_button.cget('state') == 'disabled' and replace_down_arrow_button.cget('state') == 'disabled'):
...
def FillingFiles(finished_hca_files, replace_dir):
    if finished_hca_files == replace_dir:
        move_files = False
    else:
        move_files = True
    ...
    if len(displayed_source_files) == 0 and len(displayed_replacement_files) == 0:
        ...
        compare_button.config(bg='gray', state=DISABLED)
    fill_button.config(state=DISABLED, bg='gray')
...
disabled_button_list = [remove_button, source_up_arrow_button, replace_up_arrow_button, both_sections_up_arrow_button, top_of_both_button, 
                        source_down_arrow_button, replace_down_arrow_button, both_sections_down_arrow_button, bottom_of_both_button, compare_button, fill_button]
